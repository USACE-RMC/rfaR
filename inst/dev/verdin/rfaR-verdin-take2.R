...
rfaR <- function(BestFitParams, LP3=TRUE, StageTS, Seasonality, Hydrographs, ResModel, CriticalDur, RoutingDur, ExpectedOnly=TRUE, n.bins=50, events.per.bin=20){
  # LOAD DEPENDENCIES
  require(evd)
  require(lubridate)
  require(zoo)
  ...
  Q.samp = flowFrequencySampler(bestfit=BestFitParams, LP3=LP3, Nbin=n.bins, Mevent=events.per.bin, ExpectedOnly=ExpectedOnly)
  ...
  Hydrographs[is.na(Hydrographs)] = 0
  obsVol = apply(apply(Hydrographs, 2, rollmean, k=CriticalDur), 2, max, na.rm=TRUE)
  hydroDur = nrow(Hydrographs)
  ...
  for(i in 1:nrow(Q.samp$flow)){
    ...
    for(j in 1:ncol(Q.samp$flow)){
      ...
      hydroInput = data.frame(time=1:hydroDur, flow_cfs = (Q.samp$flow[i,j] / obsVol[hydroSamps[cnt]]) * Hydrographs[,hydroSamps[cnt]])
      # hydroSamp  = as.vector(t(Hydrographs[,hydroSamps[cnt]]))
      # hydroRatio = as.numeric(Q.samp$flow[i,j]) / obsVol[hydroSamps[cnt]]
  ...
  Sbins = 1000
  Nbins = Q.samp$n.bins
  Mevents = Q.samp$events.per.bin
  Weights = Q.samp$weights
  ...
  z = qnorm(1-aepStages)
  plot(z, peakStages,type = "l", xlab = "AEP", ylab = "Stage (ft)", main = "John Martin Dam Peak Stage Frequency", axes=F, lwd=2)#, ylim=c(3840, 3895))
  axis(2);box()
  axis(1, at=rp.tic, labels=rp.lab, las=2)
  abline(h=seq(3740, 3895, by=10), col='lightgrey')
  ...
  return(list(expectedStage=data.frame(x=aepStages, y=peakStages)))
}
...
###############################################################
flowFrequencySampler <- function(bestfit, LP3=TRUE, Nbin=NULL, Mevent=NULL, ExpectedOnly=TRUE){
  if(is.null(Nbin)){
    Nbin = 50
  }
  if(is.null(Mevent)){
  ...
  if(ExpectedOnly){
    ords = stratifiedSampler(Nbins=Nbin, Mevents=ceiling(nrow(bestfit)/Nbin))
    Q = numeric(nrow(bestfit))
    if(LP3){
      for(i in 1:nrow(bestfit)){
        Q[i] = 10^qp3(pnorm(ords$normOrd[i]), bestfit[i,1], bestfit[i,2], bestfit[i,3])
      }
    }else{
      for(i in 1:nrow(bestfit)){
        Q[i] = qgev(pnorm(ords$normOrd[i]), bestfit[i,1], bestfit[i,2], bestfit[i,3])
      }
    }
    Q = matrix(sort(Q), ncol=1)
    #Q = matrix(Q, nrow=ords$Mevents, ncol=ords$Nbins)#ncol=1)
  }else{
    ords = stratifiedSampler(Nbins=Nbin, Mevents=Mevent)
    Q = matrix(NA,nrow=length(ords$normOrd), ncol=nrow(bestfit))

    if(LP3){
      for(i in 1:nrow(bestfit)){
        # for(j in 1:length(ords$normOrd)){
        #   Q[j,i] = 10^qp3(pnorm(ords$normOrd[j]), bestfit[i,1], bestfit[i,2], bestfit[i,3])
        # }
        Q[,i] = 10^qp3(pnorm(ords$normOrd), bestfit[i,1], bestfit[i,2], bestfit[i,3])
      }
    }else{
      for(i in 1:nrow(bestfit)){
        # for(j in 1:length(ords$normOrd)){
        #  Q[j,i] = qgev(pnorm(ords$normOrd[j]), bestfit[i,1], bestfit[i,2], bestfit[i,3])
        # }
        Q[,i] = qgev(pnorm(ords$normOrd), bestfit[i,1], bestfit[i,2], bestfit[i,3])
      }
    }
  }
