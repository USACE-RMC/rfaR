...
# Corresponding Month from Stage Duration Table (Jan-Dec)
# First Column is prob so add 1
jmd_stage_duration[,UniqMonths[i]+1]
jmd_stage_duration[,1]
...
stages_df_sum <- stages_df |>
  group_by(Month_fact) |>
  summarize(Stage_q25 = quantile(stages,probs = .25),
            Stage_q50 = quantile(stages,probs = .5),
            Stage_q75 = quantile(stages,probs = .75),
            Stage_q95 = quantile(stages,probs = .95))
