...
for (s in scale_labels) {
  ...
  row_diff <- abs(n_rfar - n_hms)
  print(paste("Row diff for", s, " = ", row_diff))
}
...
for (s in scale_labels) {
  ...
  print(paste("Inflow Difference for", s,
...
for (s in scale_labels) {
  ...
  # Tolerance (in feet)
  tol <- 0.05
  print(paste("Elevation Difference for", s,
...
for (s in scale_labels) {
  ...
  print(paste("Storage % Difference for", s,
              "- max diff:", round(max_pct_diff, 4), "%"))
}

# Test 5: Outflow is within 5 cfs of HMS results
...
for (s in scale_labels) {
  ...
  print(paste("Outflow Difference for", s,
              "- max diff:", round(max_diff, 4), "cfs"))
}
...
for (s in scale_labels) {
  ...
  print(paste("Peak Stage Difference for", s,
              "- max diff:", round(peak_diff, 4), "ft"))
